...
export default function AllLaunchers() {
  ...
  return (
    <>
      <div className="contaner">
        <div className="continr-serch">
          <input
            ...
            onChange={(e) => setSerce(e.target.value)}
          />
          <select
            name="rocketType"
            id="rocketType"
          ...
          <tbody>
            {filterLaunchers().map((l) => (
              <tr key={l._id} className="row">
                ...
                <td className={`${l.destroyed}`}>{`${l.destroyed}`}</td>
                <td>
                  <button
                    className="Details"
                    onClick={() => navigate(`/launcher/${l._id}`)}
                  >
                    Details
                  </button>
                </td>
      ...
      {editLauncher && (
        <UpdateLauncherComponents
          edit={editLauncher}
          setEdit={setEditLauncher}
        />
      )}
    </>
  );
